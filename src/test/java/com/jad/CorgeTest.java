...
class CorgeTest {
    ...
    void hasDoSomethingLikeACorgeMethodTest() {
        Utils.testIfMethodExists(CorgeTest.corgeClass, "doSomethingLikeACorge");
    }
    ...
    void getFooAndSetFooTest() {
        ...
        Object corgeInstance = assertDoesNotThrow(
                () -> CorgeTest.corgeClass.getDeclaredMethod("setFoo", iFooClass).invoke(mockFoo));

        // Verify that the getFoo method returns the same instance of IFoo
        Object fooValue = assertDoesNotThrow(
                () -> CorgeTest.corgeClass.getDeclaredMethod("getFoo").invoke(corgeInstance));
        assertEquals(mockFoo, fooValue, "The getFoo method should return the same instance of IFoo.");

        // Verify that the getCorge method of IFoo returns the same instance of Corge
        Object corgeValue = assertDoesNotThrow(() -> iFooClass.getDeclaredMethod("getCorge").invoke(mockFoo));
        assertEquals(corgeInstance, corgeValue, "The getCorge method should return the same instance of Corge.");
        ...
        assertDoesNotThrow(() -> CorgeTest.corgeClass.getDeclaredMethod("setFoo", iFooClass).invoke(mockFoo2));
        Object fooValue2 = assertDoesNotThrow(
                () -> CorgeTest.corgeClass.getDeclaredMethod("getFoo").invoke(corgeInstance));
